...
in the series of overtones (on the fourth fret, with the string
divided in five) and therefore sounds less strongly than the
...
progression against a sustained chord: C /b /a /g.
...
Csus4 and Cm7-5). The reason for this is that ``{}sophisticated''{}
...
that a single number (e.g. 11) indicates the \emph{last member} of the
stack to be included, not just a single tone: C11 consists of the all
the tones in the stack, up to the eleventh.
...
major and minor: B$\flat$ and B), not to the chord itself -- cf. the ``m7'' chord
below. Note also that ``7'' always refers to the minor seventh. If the
major seventh is used, it has to be indicated with ``maj7''.
...
\noindent The ninth and the seventh are added to the root
chord.
...
in a true sus4 chord, the third is not included. If that is the case,
the chord would be called add11 or add4.
...
thirds, upon which the rest of the system is based.
...
it begins in the third fret. Thus, the quality of the chord is not
...
\noindent I usually also prefer simple names to ``exact'' names. A chord
...
fuller chord, and since you can use all the strings -- unless you
precisely want the higher sound, in which case the first fingering is
...
leaves us with the alternatives x2323x, xx(3)435 or x2x231. (Note:
...
Repeats are indicated as in the previous example, or written out
(``x3'')
...
For some of the songs, Dylan uses alternate or open tunings. An open
tuning is a tuning where all the strings are tuned to a chord, whereas
alternate tunings are other ways of altering the
tuning.
...
two melody strings. The baroque lute was tuned as an open d minor
...
production) are open D, open E and open G.
...
in his early carreer, you'll find a few songs in this tuning. The only
...
Freewheelin' outtake Wichita, which I've written more extensively
about in the blog, and One too many mornings.
...
stage. It gives a very forceful fundament, thanks to the doubled C in
...
See Andrew Mullins' essay at Expecting Rain for further information on
open and alternate tunings: \url{http://www.expectingrain.com/dok/songs/tunings.html}
...
are also shooting themselves in the foot. The reason for the ``{}thumb
always behind the neck''{} rule is to ensure economy of means, maximum
of flexibility with a minimum of physical effort, but if you don't
...
the ``{}index-finger G''{} in \emph{Renaldo \& Clara }(or is it the
\emph{Hard Rain }TV special?), then go and rehearse the ``{}pinky
G''{} instead (I cringe everytime I watch that sequence).
...
`In the Garden'?'' Answer: you're not. You pick some of them, perhaps
...
243115 can be played with a ``{}twisted barre''{}, with an index
